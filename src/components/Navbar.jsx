...
import { Link } from "react-router-dom";
import logo from "/Sys.png";
import { Menu, X, ChevronDown, ChevronRight, Briefcase, Users, FolderKanban, GitBranch, Wrench, Zap, Database, Code, Cloud, LayoutGrid, ShieldCheck, UserCog } from "lucide-react";
...
const Navbar = ({ isScrolled }) => {
  ...
  const [desktopOpen, setDesktopOpen] = useState(false);         // Services dropdown
  const [desktopSub, setDesktopSub] = useState(null);            // consulting | staffing
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 py-1 flex justify-between items-center">
        <Link to="/" className="flex items-center">
          <img className="w-10 h-10 md:w-18 md:h-18 rounded-full" src={logo} alt="Logo" />
        </Link>

        {/* ── Desktop Navigation ── */}
        <div className="hidden md:flex space-x-8 items-center">
          <Link to="/" className="text-gray-700 hover:text-blue-600 transition font-medium hover:underline underline-offset-4">
            Home
          </Link>

          {/* Services — click OR hover both work */}
          <div
            ref={servicesRef}
            className="relative"
            onMouseEnter={() => setDesktopOpen(true)}
            onMouseLeave={() => { setDesktopOpen(false); setDesktopSub(null); }}
          >
            <button
              onClick={() => { setDesktopOpen((v) => !v); setDesktopSub(null); }}
              className="flex items-center text-gray-700 hover:text-blue-600 transition font-medium hover:underline underline-offset-4"
            >
              Services
              <ChevronDown
                size={16}
                className={`ml-1 transition-transform ${desktopOpen ? "rotate-180" : ""}`}
              />
            </button>

            {desktopOpen && (
              <div className="absolute left-0 flex flex-col bg-white shadow-xl border border-gray-200 rounded-xl mt-1 py-2 px-4 w-64 z-50 animate-fade-in">

                {/* IT Consulting row */}
                <div
                  className="relative"
                  onMouseEnter={() => setDesktopSub("consulting")}
                  onMouseLeave={() => setDesktopSub(null)}
                >
                  <div className="flex items-center justify-between px-2 py-2 text-sm font-semibold hover:bg-gray-50 hover:text-blue-600 rounded-md transition">
                    <Link
                      to="/services/it-consulting"
                      onClick={closeAll}
                    ...
                    <button
                      onClick={(e) => { e.preventDefault(); setDesktopSub(desktopSub === "consulting" ? null : "consulting"); }}
                      className="p-1"
                    >
                      <ChevronRight
                        size={16}
                        className={`transition-transform ${desktopSub === "consulting" ? "rotate-90" : ""}`}
                      />
                    </button>
                  </div>

                  {desktopSub === "consulting" && (
                    <div className="absolute left-full top-0 ml-1 flex flex-col bg-white shadow-xl border border-gray-200 rounded-xl py-2 px-4 w-56 z-50">
                      {serviceItems.consulting.map((item, i) => (
                        <Link
                          key={i}
                          to={item.path}
                          onClick={closeAll}
                ...
                <div
                  className="relative"
                  onMouseEnter={() => setDesktopSub("staffing")}
                  onMouseLeave={() => setDesktopSub(null)}
                >
                  <div className="flex items-center justify-between px-2 py-2 text-sm font-semibold hover:bg-gray-50 hover:text-blue-600 rounded-md transition">
                    <Link
                    ...
                    <button
                      onClick={(e) => { e.preventDefault(); setDesktopSub(desktopSub === "staffing" ? null : "staffing"); }}
                      className="p-1"
                    >
                      <ChevronRight
                        size={16}
                        className={`transition-transform ${desktopSub === "staffing" ? "rotate-90" : ""}`}
                      />
                    </button>
                  </div>

                  {desktopSub === "staffing" && (
                    <div className="absolute left-full top-0 ml-1 flex flex-col bg-white shadow-xl border border-gray-200 rounded-xl py-2 px-4 w-64 z-50">
                      {serviceItems.staffing.map((item, i) => (
                        <Link
                          key={i}
                          to={item.path}
                          onClick={closeAll}
          ...
          <Link to="/clients" className="text-gray-700 hover:text-blue-600 transition font-medium hover:underline underline-offset-4">Clients</Link>
          <Link to="/about-us" className="text-gray-700 hover:text-blue-600 transition font-medium hover:underline underline-offset-4">About us</Link>
          <Link to="/contact" className="text-gray-700 hover:text-blue-600 transition font-medium hover:underline underline-offset-4">Contact</Link>
          <Link to="/contact" className="ml-4 bg-blue-600 hover:bg-blue-700 text-white py-2 px-5 rounded-full transition duration-300 shadow-sm">
            Get Started
          </Link>
        </div>
      ...
      {isOpen && (
        <div className="md:hidden bg-white shadow-md absolute top-full left-0 right-0 transition-all duration-300 animate-slide-down z-40 max-h-96 overflow-y-auto">
          <div className="flex flex-col px-6 py-4 space-y-3">
            <Link to="/" onClick={() => setIsOpen(false)} className="text-gray-700 hover:text-blue-600 font-semibold">Home</Link>
            ...
            <div>
              ...
              <div className="ml-4 mt-2">
                <button
                  ...
                  className="w-full text-left text-sm text-gray-600 hover:text-blue-600 font-semibold flex items-center justify-between"
                >
                  <span className="flex gap-1"><Briefcase /> IT Consulting</span>
                  <ChevronDown size={16} className={`transition-transform ${openSubmenu === "consulting" ? "rotate-180" : ""}`} />
                </button>
                {openSubmenu === "consulting" && (
                  <div className="ml-6 mt-2 space-y-2">
                    {serviceItems.consulting.map((item, i) => (
                      <Link key={i} to={item.path} onClick={() => setIsOpen(false)} className="block text-xs text-gray-500 hover:text-blue-600 py-1">
                        • {item.name}
                      </Link>
                    ))}
                  </div>
              ...
              <div className="ml-4 mt-2">
                <button
                  ...
                  className="w-full text-left text-sm text-gray-600 hover:text-blue-600 font-semibold flex items-center justify-between"
                >
                  <span className="flex gap-1"><Users /> IT Staffing</span>
                  <ChevronDown size={16} className={`transition-transform ${openSubmenu === "staffing" ? "rotate-180" : ""}`} />
                </button>
                {openSubmenu === "staffing" && (
                  <div className="ml-6 mt-2 space-y-2">
                    {serviceItems.staffing.map((item, i) => (
            ...
            <Link to="/clients" onClick={() => setIsOpen(false)} className="text-gray-700 hover:text-blue-600 font-semibold">Clients</Link>
            <Link to="/about-us" onClick={() => setIsOpen(false)} className="text-gray-700 hover:text-blue-600 font-semibold">About us</Link>
            <Link to="/contact" onClick={() => setIsOpen(false)} className="text-gray-700 hover:text-blue-600 font-semibold">Contact</Link>
            <Link to="/contact" onClick={() => setIsOpen(false)} className="bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded-full text-center transition">
              Get Started
            </Link>
          </div>
        </div>
